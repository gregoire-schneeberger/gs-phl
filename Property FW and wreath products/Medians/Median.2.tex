...
Let $G$ and $H$ be two groups with $G$ non-trivial and let $X$ be a set on which $H$ acts. The wreath product $G \wr_X H$ has property (T) if and only if $G$ and $H$ have property (T) and $X$ is finite.
\end{thm}
%
%
...
By adopting the point of view of actions with bounded orbits, we obtain an unified proof of the following.
%
%
...
They are sometimes called \emph{standard wreath products} or simply \emph{wreaths products}, while general $G\wr_XH$ are sometimes called \emph{permutational wreath products}.
The probably most well-kwon examples of wreath product is the so called \emph{lamplighter group} $(\Z/2\Z)\wr_\Z\Z$.
...
%
%
Remind that a metric space $(X,d)$ is \emph{ultrametric} if and only if for any $x$, $y$ and $z$ in $X$ we have $d(x,y)\leq\max\{d(x,z),d(z,y)\}$.
...
A graph is \emph{median} if each of its connected components is median. For surveys on median graphs see \cite{MR1705337,MR2405677}.\todo{Maybe add refs with respect to FW.}
...
We have the following strict implications between the properties of Definition~\ref{Def:FHFA} \cite{MR1432323,MR0476875 ,MR3299841,2013arXiv1302.5982C}
...
On the other hand, we will also use the following result on groups with cof~$\neq\omega$:
%
%
...
%
%
In other words, an additional structure on metric spaces, is concrete category over $\mathbf{Met}$. Since $\mathbf{Met}$ itself is concrete, that is we have a faithful functor $F\colon \mathbf{Met}\to\mathbf{Set}$, the category $\mathbf{SMet}$ is also concrete and its objects can be thoughts as set with ``extra structure''.

Obvious examples of additional structures on metric spaces include, metric spaces, Hilbert spaces and ultrametric spaces.
For connected graphs (and hence for connected median graphs and for trees), one look at the category $\mathbf{Graph}$ where objects are connected simple graphs $G=(V,E)$ and where a morphism $f\colon (V,E)\to(V',E')$ is a function between the vertex sets such that if $(x,y)$ is an edge then either $f(x)=f(y)$ or $(f(x),f(y))$ is an edge.
...
\begin{enumerate}
\item
The projections $\pi_i\colon X^n\to X$ are S-homomorphisms,
\item\label{Condidef:1}
The underlying metric space $F_S(X^n)$ is a product metric space, that is its underlying topology is the product topology on $X^n$,
...
%
%
It follows from Theorems~\ref{Thm:FACK} and \ref{Thm:UncCoun} than in the assumptions of Lemma~\ref{Lemma:XFinite} it is not possible to replace property FW by property FA or by cof~$\neq\omega$.
%
%
...
We then define $g.(x_1,\dots,x_n)\coloneqq(h_{g,g^{-1}.1}.x_{g^{-1}.1},\dots,h_{g,g^{-1}.n}.x_{g^{-1}.n})$. This is indeed an action, which, by Conditions~\ref{Condidef:3} and \ref{Condidef:4} of Definition~\ref{Def:Cartesian}, is by S-automorphisms\todo{check that}, of $X^{G/H}$.
...
\]
By Conditions~\ref{Condidef:3} and \ref{Condidef:4} of Definition~\ref{Def:Cartesian}, this action is by S-automorphisms.\todo{check that}
