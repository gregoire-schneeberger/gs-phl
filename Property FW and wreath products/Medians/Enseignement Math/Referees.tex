...
More precisely, we need a variation of (A3) for countable Cartesian products (for morphisms we only ask that $\bigoplus_{i\in\N} \Aut_\textbf{S}(X_i)\subseteq\Aut_\textbf{S}(\bigoplus_{i\in\N} X_i)$) and a strong version of (A1) saying that there exists an universal bound $M$ such that any non-trivial group $G$ acts on some \textbf{S}-space moving a point at distance at least $M$.
