...
\end{itemize} \GS{Rendu cohérent la nomenclature, toujours : "any action by isometries"}
\end{defn}
%
%
...
\end{align*}\GS{rendu cohérent, tu préfères $(f,g)$ ou $((x_i),(y_i))$ ?}
...
Then for every integer $n$, the orbit $K.1_K$ contains $\{g_1,\dots,g_n,1,\dots\}$ which is at distance $n$ of $1_K$ for $d_{\max}$ if the $g_i$ are not equal to $1$.\GS{Expliciter qui est $+1_K$}
...
Suppose that \BS{} implies having uncountable cofinality.
Let $G$ and $(G_x)_{x\in X}$ be non-trivial groups. \GS{Idem remarque ci-dessus pour $H$}Then
\begin{enumerate}
