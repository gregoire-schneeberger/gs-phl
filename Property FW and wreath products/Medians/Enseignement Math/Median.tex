...
A subcategory \CatS{} of \PMet{} has \emph{non-trivial group actions} if for every non-trivial group $G$ there exists an \CatS-space $X$ with at least two elements which admits a non-trivial action of $G$.
\end{defn}
%
%
Examples of categories \CatS{} with non-trivial group actions include (pseudo-) metric spaces (with $X$ a Cayley graph of $G$), (reflexive) Banach spaces and Hilbert spaces (with $X=l^2(G)$), connected median graphs and (real) trees ($X$ has one central vertex to which we attach an edge for every $g\in G$).
%
%
