...
%\GS{Ajout références liens entre cube et wall spaces}
...
One can also want to look at the category of all Banach spaces (the corresponding property~BB hence stands between the Bergman's property and property~\FB)\GS{BB Etudiée quelque part ? red?}, or the category of $L^p$-spaces for $p$ fixed~\cite{MR2978328} (if $p\notin\{1,\infty\}$, then B$L^p$ is implied by \FB).
...
Let $G$ be a group and $A$ an $B$ be two subgroups such that $G=AB$.\GS{cf mail pour $G=AB$}
...
Finally, let $h\in G$\GS{Je ne suis jamais très fan d'avoir un élément $h$ qui est dans $G$ alors qu'il y a un groupe $H$, peut être l'appeler $g_0$ ?} be an element of unbounded $G$-orbit for $d$ and let $x_0$ be any element of $X$.
For $g$ in $G$ and $x$ in $X$, we define the following analogs of Kronecker's delta functions
\begin{equation*}
\delta_x^g (y) \coloneqq
\begin{cases}
...
On the other hand, the diameter of $\bigoplus_XG.y$ pour ne pas  is bounded by the supremum of the diameters of the $G_{x}.y$, and hence bounded by $\max\{D_0,D_1,\dots,D_n\}$.
Finally, for $(\varphi,h)$ in $G\wr_YH$ \GS{$G\wr_XH$ plutôt ?}we have
\begin{align*}
