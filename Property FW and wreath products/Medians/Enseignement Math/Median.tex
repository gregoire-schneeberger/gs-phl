...
And if $\sigma(l,m,n)\coloneqq\frac1l+\frac1m+\frac1n\leq 1$, then $\Delta(m,l,n)$ is the infinite symmetric group of a tilling of the Euclidean plane (if $\sigma(m,l,n)=1$) or of the hyperbolic plane (if $\sigma(m,l,n)<1$) and hence acts on a space with walls without fixed points, which implies that it doesn't have property FW.
...
One can also want to look at the category of all Banach spaces (the corresponding property~\BS{} hence stands between the Bergman's property and property~\FB), or the category of $L^p$-spaces for $p$ fixed~\cite{MR2978328} (if $p\notin\{1,\infty\}$, then \BS{} is implied by \FB).
...
\end{align*}
with the convention that an uncountable sum is infinite.
\end{defn}
%
%
...
A subcategory \CatS{} of \PMet{} has \emph{finite bornological Cartesian powers} if for any \CatS-space $X$ and any integer $n$, there exists an \CatS-object, called a \emph{$n$\textsuperscript{th} Cartesian power of $X$} and written $X^n$, such that:
\begin{enumerate}
\item
...
For graphs, the usual Cartesian product, which corresponds to $d_1=d_X+d_Y$ works well, but one can also take the strong product (which is the categorial product in \category{Graph}) that is the distance $d_\infty$.
For connected median graphs, we take the usual Cartesian product.\footnote{The category of median graphs does not have categorial products.}
On the other hand, (real) trees do not have Cartesian powers, even without asking them to respect the bornology.

As the above examples illustrate, there can be multiple non-isomorphic spaces playing the role of a Cartesian power. As our results will not depend on a particular choice of a Cartesian power, we will sometimes make a slight abuse of language and speak of \emph{the} Cartesian power.
...
\begin{align*}
	...
	\sideset{}{^p}\bigoplus_{i \in I}X_i&\coloneqq\setst{f\in I}{d_p(f,f_0)<\infty}.
\end{align*}
...
Moreover, if $\Aut_\CatS(X)$ acts transitively on $X$, then $\bigoplus_{i \in I}^pX_i$ will not depend on the choice of $x\in X$.
...
\item
The canonical image of $\Aut_{\CatS}(X)\wr_I \Sym(n)$ in $\Bij(\prod_I X)$ lies in $\Aut_{\CatS}(\bigoplus_I^\CatS X)$,
\item
For any $y\neq x_0$, the following set has infinite diameter 
%$\setst{f\in \bigoplus_I X}{f(x)=x_0 \textnormal{ for all but finitely many } x \textnormal {for which } f(x)=y}$
\[
	\setst{f\in \bigoplus_I X}{f(x)=y \textnormal{ for finitely many } x \textnormal{ and otherwise }f(x)=x_0}.
\]
...
In particular, the following categories have separating infinite  Cartesian powers: (pseudo-) metric spaces, (median) graphs, (reflexive) Banach spaces and Hilbert spaces.
%
%
%
...
In Subsection~\ref{SubSec:CatS}, we defined $3$ axioms that \CatS{} might satisfy. Axiom (A1) simply state that a non-trivial group acts non-trivially on some \CatS-space. Axioms (A2) and (A3) guarantee the existence of finite and infinite Cartesian powers, which should be compatible in some sense with the bornology. In Table~\ref{Table1} we present a short reminder on whenever these axioms are satisfied for some properties that were mentioned in Sections~\ref{Section:Intro} and~\ref{Section:Def}.
\begin{table}
\begin{tabular}{*{5}{|c}|}
\hline
Category \CatS & Group property & (A1) & (A2) & (A3)\\
\hline
\hline
...
\hline
Z$=2$-regular tree with $\Isom^+$	&BZ	&\xmark	&\xmark	&\xmark\\
\hline
Spaces of finite diameter&BF		&\checkmark&\checkmark&\xmark\\
\hline
\end{tabular}
...
We now have two easy but useful results.
%
%
...
More precisely, for Corollary~\ref{Cor:Cof}.1 we need the existence of infinite Cartesian products (not necessarily separating) and a strong version of (A1) saying that there exists an universal bound $M$ such that any non-trivial group $G$ acts on some \CatS-space moving a point at distance at least $M$.
...
As a direct corollary, we obtain that if property \BS{} implies property \FW, then the conclusion of Lemma~\ref{Lemma:XFinite} holds even if \CatS{} might not satisfy it's premises.
...
Suppose that \CatS{} has non-trivial group actions finite bornological Cartesian powers and seperating infinite Cartesian powers.
