...
%
%
\begin{lem}\label{Lemma:Cof}%\GS{Ajouter $H$ un groupe quelconque agissant sur $X$ ?}
%Suppose that \BS{} implies having uncountable cofinality.
Let $G$ and $(G_x)_{x\in X}$ be non-trivial groups and let $H$ be a group acting on $X$.\PH{Ajouté H...}
 Then
\begin{enumerate}
...
Then for every integer $n$, the orbit $K.\{1,1,\dots\}$\PH{Plutôt que de déf $1_K$ j'ai opté pour la notation explicite} contains $\{g_1,\dots,g_n,1,\dots\}$ which is at distance $n$ of $\{1,1,\dots\}$ for $d_{\max}$ if the $g_i$ are not equal to $1$.
