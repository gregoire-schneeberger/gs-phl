...
The corresponding result for property~\FA{} (every action on a tree has bounded orbits) and property \FR{} (every action on a real tree has bounded orbits),\GS{Ajout définition de \FR comme on en parle dans le Thm et que ce n'est pas définit, on peut y enlever si tu préfères} is a little more convoluted and was obtained a few years later by Cornulier and Kar.
%
%
...
One implication is trivial, as finite Abelian groups\GS{Pourquoi une majuscule à "Abelian group", il me semble que cela devrait plutôt être "abelian group" ici et partout ?} do not project onto $\Z$.
For the other implication, suppose that $G$ has uncountable cofinality but infinite abelianization $G/[G,G]$. The group $G/[G,G]$ being an infinite Abelian group, it has a countably infinite quotient $A$ --- a classical fact that Y. de Cornulier kindly reminded us, see \cite{MR551496}[§16.11.c] for a proof. The quotient $A$ has uncountable cofinality, see Lemma~\ref{Lemma:Quotient}, and is therefore an infinite finitely generated Abelian group, which hence projects onto $\Z$.
...
The class of median graphs was introduced by Nebesk\'y in 1971 \cite{Neb71} and Gerasimov \cite{Ger97, MR1663779},   Roller \cite{Rol98} and Chepoï \cite{Che00} realized independently that this class coincides with the class of $1$-skeleta\GS{$1$-skeleta ou $1$-skeleton (que je trouve plus dans les ref)?} of CAT(0) cube complexes.
...
It implies in particular that a countable group has uncountable cofinality if and only if it is finitely generated.\GS{C'est clair ça ? peut être mettre l'idée du pourquoi ?}
%
%
...
The implication $[\textnormal{\FH}\implies \textnormal{\FR}]$ follows from the fact that real trees are median metric spaces\GS{space -> spaces}, and that such spaces can be embedded into $L^1$-spaces (see for instance \cite[Theorem V.2.4]{Ver93}).
...
%
%
The reader familiar with triangles groups $\Delta(l,m,n)=\langle a,b,c\,|\,a^2=b^2=c^2=(ab)^l=(bc)^m=(ca)^n=1\rangle$ with $l,m,n\in\{1,2,\dots\}\cup\{\infty\}$ will be happy to observe that they provide explicit examples of groups with property~\FA{} but not property~\FW.
...
And if $\kappa(l,m,n)\coloneqq\frac1l+\frac1m+\frac1n\leq 1$, then $\Delta(l,m,n)$ is the infinite symmetric group of a tilling of the Euclidean plane (if $\kappa(l,m,n)=1$) or of the hyperbolic plane (if $\kappa(l,m,n)<1$) and hence acts on a space with walls without fixed point, which implies that it does not have property FW.\GS{J'ai juste changé l'ordre des variables pour avoir $(l,m,n)$ partout}
...
If $G$ is a group and $H$ a subgroup of~$G$, we say that the pair $(G,H)$ has \emph{relative property~\BS} if for every $G$-action\GS{tous les "$G$ action" -> "$G$-action" pour être coherent dans tout le texte, idem pour $H$-action} on an \CatS-space, the $H$ orbits are bounded.
...
%
%
Observe that an alternative proof of the above Lemma can be easily deduced from the following characterization of Bergman's property due to Cornulier~\cite{MR2240370}: a group $G$ has Bergman's property if and only if it has uncountable cofinality and for every generating set $T$ of $G$, the Calyey graph $\Cayley(G;T)$\GS{Changé la notation de graphes de Cayley, "Cayl" -> "Cay"} is bounded.
Details are left \GS{let -> left} to the reader.
...
\end{equation*}\GS{On pourrait ajouter "for $p \in $?"}
...
The details are left\GS{let -> left} to the interested reader.
%
%
%
...
Suppose that \BS{} implies having uncountable cofinality.
Let $G$ and $(G_x)_{x\in X}$ be non-trivial groups and let $H$ be a group acting on $X$. %\PH{Ajouté H...}% \GS{Idem remarque ci-dessus pour $H$}Then
\begin{enumerate}
