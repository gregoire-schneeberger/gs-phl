...
Our main result is that for many\PH{``a large class of''$\to$``many''} subcategories $\mathbf{S}$, the wreath product $G\wr_XH$ has property \BS{} if and only if both $G$ and $H$ have property \BS{} and $X$ is finite. On one hand, this encompasses in a general setting previously known results for properties \FH and \FW. On the other hand, this also applies to the Bergman's property.
...
%
%
It is straightforward to prove that the wreath product $G \wr_X H$ is finitely generated if and only both $G$ and $H$ are finitely generated and if the number of orbits of the action of $H$ on $X$ is finite.\GS{ajout remarque}
