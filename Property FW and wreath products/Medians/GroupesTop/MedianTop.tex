...
%
\begin{abstract}
If \CatS{} is subcategory of metric spaces, we say that a group G has property~\BS{} if any isometric action on a \CatS-space has bounded orbits.
...
Historically many\PH{Ajout ``many''. uncountable cof n'est pas une propriété de points fixes} of these properties were defined using the existence of fixed points, but in some cases this is equivalent.

Our main result is that for many\PH{``a large class of''$\to$``many''} subcategories \CatS, the wreath product $G\wr_XH$ has property~\BS{} if and only if both $G$ and $H$ have property~\BS{} and $X$ is finite.
...
%
%
It is straightforward to prove that the wreath product $G \wr_X H$ is finitely generated if and only both $G$ and $H$ are finitely generated and if the number of orbits of the action of $H$ on $X$ is finite.
Y. Stalder has let us know (private communication) that the arguments of~\cite{LS2020} can be adapted to replace the finite generation hypothesis of Theorem~\ref{Thm:PropFW} by the condition that all three of $G$, $H$ and $X$ are at most countable.\GS{Est ce que on peut remplacer par "$G$, $H$ and $X$ are at most countable" ?}\PH{Oui. Fait.}
In particular, Theorem~\ref{Thm:PropFW} covers the result of A. Genevois published in~\cite{2017arXiv170500834G}.\PH{Il faudrait sans doute réécrire le §. pour mettre Cornulier plus en avant...}
...
\item
\emph{Property \FR} if any isometric action on a metric space on a real tree has bounded orbits,
\item
...
In the following, we will often do a slight abuse of notation and simply speak of a \emph{group action} on a space $X$, without always specifying by which kinds of maps the group acts, which should always be clear from the context.\GS{Sens de ce paragraphe. Est ce utile ?}\PH{Ça évite de à chaque fois préciser ``$G$ agit par isométries affines sur un Hilbert'' (par ex) et de simplement dire ``$G$ agit sur un Hilbert''.}
...
However this is equivalent with the bounded orbit property see Proposition~\ref{Proposition:Mediane} and the discussion below~it. \GS{Ajout de ce § comme une remarque}
\PH{Je trouve que c'est mieux en le laissant en §, afin d'éviter de surcharger. De toute façon, on en discute en long et en large plus tard. Mais si tu veux revenir à la remarque, je ne me battrai pas.}
%\end{rem}
...
For countable groups (and more generally for $\sigma$-compact locally compact groups), property~\FH{} is equivalent to the celebrated Kazhdan's property (T) by the Delorme-Guichardet theorem, but this is not true in general.
...
It is intended as a note for the interested reader, and can be skipped without any harm.\PH{Réécrire l'intro}
...
%Heuristically, a \CatS-space is a (quasipseudo-) metric space with an additional structure (as for example an Hilbert space). For a precise definition, see Definition~\ref{Def:PropBS}.
...
\end{lem}
\begin{proof}
We have $H\cong G/N$. If $H$ acts continuously\PH{J'ai réécrit la preuve} on some \CatS-space $X$ with an unbounded orbit, then the surjection $G\twoheadrightarrow H$ gives us a continuous $G$ action on~$X$, with the same orbits as the $H$ action.
\end{proof}
%
%
...
\end{lem}
\begin{proof}
Let $X$ be a \CatS-space on which $G$ acts continuously and let $x$ be an element of $X$.
