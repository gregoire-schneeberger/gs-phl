...
%
\begin{defn}
Let $S=(\mathbf{SBorn},F_S)$ be a concrete category over $\mathbf{Born}$.
A S-object $X$ is \emph{uniform} if for any bounded set $A$ in $\B$ and any collection of S-morphisms $\setst{f_i\colon X\to X}{i\in I}$, if the set $\bigcup_{i\in I}f_i(A)$ is unbounded then there exists $x|in A$ such that $\setst{f_i(x)}{i\in I}$ is unbounded.
%A bornological space $(X,\B)$ is \emph{uniform} if for any bounded set $A$ in $\B$ and any collection of bounded maps $\setst{f_i\colon X\to X}{i\in I}$, if the set $\bigcup_{i\in I}f_i(A)$ is unbounded then there exists $x|in A$ such that $\setst{f_i(x)}{i\in I}$ is unbounded.
\end{defn}
%
%
Observe that the uniformity of $X$ depends a priori on the type of morphisms considered.
...
%
\begin{defn}
A concrete category $(\mathbf{SBorn},F_S)$ over $\mathbf{Born}$ is \emph{uniform} if all its objects are uniform.
\end{defn}
%
%
...
On the other hand, if $(X,d)$ is a quasipseudo-metric space, then $\diam(\bigcup_{i\in I}f_i(A))$ is bounded above by $2\diam(A)+\diam(\setst{f_i(x)}{i\in I})$ for any $x\in A$.
...
Indeed, let $\B$ be the weakest bornology on $\R^2$ containing all the lines. That is, a subset of $\R^2$ is bounded if and only if it is contained in a union of finitely many lines.
Then translations $\tau_r\colon x\mapsto x+(r,0)$ are isomorphisms, $A=\setst{(0,y)}{y\in \R}$ is bounded, for every $x$ in $A$ the set $\bigcup_{r\in\R}\tau_r(x)$ is a line and hence bounded while $\bigcup_{r\in\R}\tau_r(A)=\R^2$ is unbounded.
