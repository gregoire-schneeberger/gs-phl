...
The reason behind this is the forthcoming Lemma~\ref{Lemma:Semidirect}, which fails for general bornological spaces.\PH{Je pense qu'on peut en rester là et ne pas parler de l'exemple $\{$bornés$\}=\{$finis$\}$.}
\end{rem}
%
%
...
If both $A$ and $B$ have property \BS, so has $G$.
\end{lem}
\begin{proof}
...
Moreover, since $A$ acts by isometries, all the $a.Bx$ have diameter $D_1$.
Let $y$ be an element of $G.x$. There exists $a\in A$ such that $y$ belongs to $a.Bx$. Since $1$ belongs to $B$, $y$ is at distance at most $D_1$ of $a.x$ and hence at distance at most $D_1+D_2$ of~$x$.
\end{proof}
%
%
