...
We now remind the definition of various properties for actions on metric spaces.\PH{Peut-être mettre une phrase avec les réf de première fois où ça été étudié. Soit avant la déf, soit dans le paragraphe après} \GS{On a mis déjà pas mal de références à mon avis...}
%
%
...
%
%
We conclude this section by a remark\PH{Peut-être à enlever de l'article, non ? Mais à garder pour ta thèse ?}\GS{A voir, de manière générale j'aime assez quand on traite les questions qui apparaissent raisonnablement... A toi de voir !} on a variation of Definition~\ref{Def:FHFA}.
...
If both $(G,A)$ and $(G,B)$ have relative property \BS, then $G$ has property \BS.\GS{Pourquoi utiliser rel prop \BS et pas directement $A,B$ possèdent \BS?}
\end{lem}
\begin{proof}
...
Let $y$ be an element of $G.x$. There exists $a\in A$ such that $y$ belongs to $a.Bx$. Since $1$ belongs to $B$, $y$ is at distance at most $D_1$ of $a.x$. Altogether we have that the diameter of $G.x$ is bounded above by $2D_1+D_2$. \end{proof}
%
\begin{lem}\label{Lemma:InterProd}
Let $G$ be a group and $A$ an $B$ be two subgroups such that $G=AB$.
...
Since $N$ is a subgroup of isometries of $X$, the map $d'\colon X/N\times X/N\to\R$ defined by $d'([x],[y])\coloneqq\inf\setst{d(x',y')}{x'\in N.x,y'\in N.y}$ is the quotient quasipseudo-distance on $X/N$. The triangle inequality stays not true in general for a quotient of $X$, but we leave it to the reader to verifiy that it is true in this case. \GS{Une remarque comme ça ?}%\PH{En général, pour $X/\sim$, $d'$ ne satisfait pas l'inégalité triangulaire, mais c'est vrai pour $X/N$. Est-ce qu'il faut expliciter ?}
