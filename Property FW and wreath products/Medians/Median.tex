...
\newcounter{mycomment}
\newcommand{\mycomment}[2][]{\refstepcounter{mycomment}{\todo[color={green!33},inline]{\textbf{Commentaire [\uppercase{#1}\themycomment]:}~#2}}}
...
A slightly less common operation, but still of great use in geometric group theory, is the wreath product which stands in-between the direct and the semi-direct product, see Definition~\ref{Def:WreathProd}.
...
In a recent note, the authors proved an analogous of Theorem~\ref{T:Wreath_prop_T} for property FW:
%
%
...
%
%
\todo[inline]{Réécrire ce qui suit}
The above results on property (T), FW and FA were obtained with distinct methods even if the final results share a common flavor.\todo{Retravailler la phrase, plus éventuellement dire un mot sur les différentes méthodes.}
For countable groups, property (T) is equivalent, by the Delorme-Guichardet's Theorem,\mycomment[GS]{Ref ajoutée} to property FH, see \cite[Thm. 2.12.4]{Bekka2008}. Hence, all three of this properties admit a characterization in terms of the existence of a fixed point for some actions.
... 
...
\item
\emph{uncountable cofinality} if any action on an ultrametric space has bounded orbits.
\end{itemize}
\end{defn}
\mycomment[GS]{Peut être ajouter quelque part que les actions doivent être des iso}
%
%
...
On the other hand, SB stands for Strongly Bounded and is also called the Bergman property.
Finally, a group has uncountable cofinality (sometimes written cof $ \neq\omega$) if and only if it cannot be written has a countable increasing union of proper subgroups, see Lemma~\ref{Lemma:CofSub}.
...
\begin{equation}\tag{\dag}\label{EQ:Implications}
	\textnormal{SB}\implies \textnormal{FH}\implies \textnormal{FW}\implies \textnormal{FA}\implies\textnormal{uncountable cofinality}.
\end{equation}
...
An \emph{additional structure on metric spaces} is, a category $\mathbf{SMet}$ together with a faithful functor $F\colon \mathbf{SMet}\to \mathbf{Met}$, where $\mathbf{Met}$ is the category of metric spaces.
The objects of $\mathbf{SMet}$ are called \emph{S-spaces}.
A \emph{$G$-action on a S-space} $X$ is simply an homomorphism $\alpha\colon G\to\Isom(X,X)$. It has \emph{bounded orbits} if $F\circ \alpha\colon G\to \Isom(F(X),F(X))$ has bounded orbits.
...
We now turn our attention to properties that behave well under products. Let us first introduce some definitions.\todo{On peut peut-être se contenter de \emph{compatible with powers}.}
%
%
\begin{defn}
...
We then define $g.(x_1,\dots,x_n)\coloneqq(h_{g,g^{-1}.1}.x_{g^{-1}.1},\dots,h_{g,g^{-1}.n}.x_{g^{-1}.n})$. This is indeed an action, which preserves the P-structure\todo{C'est la partie tricky (mais qui marche pour SB, FH, FW, cof$\neq \omega$). À voir si on veut dire qqch dessus} of $X^{G/H}$ and such that every element $h\in H$ acts diagonally by $h.(x_1,\dots,x_n)=(h.x_1,\dots,h.x_n)$.
...
Then $(Y^X,d)$ is a S-space and we have the \emph{primitive action}\todo{pareil, vérifier que c'est bien une action de S-space} of the wreath product $G\wr_XH$ on $Y^X$:
\[
...
\input{Definitions.Old.tex}
\input{Proofs.Old.tex}
%
%
%
%
