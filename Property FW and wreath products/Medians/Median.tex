...
%
\begin{abstract}
If $\category{S}$ is subcategory of metric spaces, we say that a group G has property \BS{} if any action on a $\category S$-space has bounded orbits. Examples of such subcategories include metric spaces, affine real Hilbert spaces, CAT(0) cube complexes, connected median graphs, trees or ultra-metric spaces.
...
Our main result is that for many\PH{``a large class of''$\to$``many''} subcategories $\category{S}$, the wreath product $G\wr_XH$ has property \BS{} if and only if both $G$ and $H$ have property \BS{} and $X$ is finite. On one hand, this encompasses in a general setting previously known results for properties (FH) and (FW). On the other hand, this also applies to the Bergman's property.
...
A $\category{S}$-space is a metric space with an ``additional structure'' and we will said that a group $G$ has \emph{property \BS{}} if every action by isometries which preserve the structure on a $\category{S}$-space has bounded orbits. Formally, this means that $\category{S}$ is a subcategory of the category of (pseudo-) metric spaces, and that the actions are by $\category{S}$-automorphisms.
...
%
%
For countable groups (and more generally for $\sigma$-compact locally-compact topological groups), property~(T) is equivalent, by the Delorme-Guichardet's Theorem, to property~(FH) (every action on affine real Hilbert space has bounded orbits), see \cite[Thm. 2.12.4]{Bekka2008}. Hence, Theorem~\ref{T:Wreath_prop_T} can also be viewed, for countable groups, as a result on property~(FH).
...
%
%
Y. Stalder has let us know (private communication) that the arguments of \cite{LS2020} can be adapted to replace the finite generation hypothesis of Theorem~\ref{Thm:PropFW} by the condition that all three of $G$, $H$ and $G\wr_XH$ are at most countable.
In particular, Theorem~\ref{Thm:PropFW} covers the result of A. Genevois published in \cite{2017arXiv170500834G}.\PH{Il faudrait sans doute réécrire le §.}
...
In particular, some of our results do not work for trees and property~(FA), neither for real trees and the corresponding property~(F$\category R$).
...
Then $G\wr_XH$ has property~(FA) (respectively has property~(F$\category R$)) if and only if $H$ has property~(FA) (respectively has property~(F$\category R$)), $H$ acts on $X$ with finitely many orbits, $G$ has no quotient isomorphic to $\Z$ and can not be written as a countable increasing union of proper subgroups.
\end{mainthm}
%
%
%
...
\item
\emph{Property (F$\category{R}$)} if any isometric action on a metric space on a real tree has bounded orbits,
\item
...
However, for graphs this is not the case; a $2$-regular tree has automorphism group $\Z\rtimes(\Z/2\Z)$, while the automorphism group of its geometric realization (the line $\category R$) is $\R\rtimes(\Z/2\Z)$. In other words, an isometry of a graph is not necessarily a graph isomorphism.
...
%&\textnormal{(F$\category{R}$)}\arrow[u]&
%\end{tikzcd}
\begin{tikzcd}[arrows=Rightarrow]
\parbox{4.5em}{\textnormal{Bergman's property}}\arrow[r]	&\textnormal{\FB} \arrow[r]	&\textnormal{(FH)} \arrow[r]	&\textnormal{(FW)}\arrow[d] &\\%
&&\textnormal{(F$\category{R}$)}\arrow[r]& \textnormal{(FA)} \arrow[r]& \parbox{5em}{\textnormal{uncountable cofinality}}
\end{tikzcd}
\end{equation}
...
\end{prop}
\begin{proof}
The implications $[\textnormal{Bergman's property}\implies \textnormal{\FB}\implies\textnormal{(FH)}]$ and $[\textnormal{(FW)}\implies \textnormal{(FA)}\Longleftarrow\textnormal{(F$\category{R}$)}]$ trivially follow from the fact that Hilbert spaces are reflexive Banach spaces, which are themselves metric spaces and that trees are both real trees and connected median graphs.
...
Finally, Minasyan constructed examples of groups with (FA) but without (F$\category R$) in \cite{MR3465847}.
\end{proof}
%
%
It is possible to consider relative versions of the properties appearing in Definition~\ref{Def:FHFA}.
If $G$ is a group and $H$ a subgroup of $G$, we say that the pair $(G,H)$ has \emph{relative property \BS} if for every $G$ action on a $\category{S}$-space, the $H$ orbits are bounded.
...
The subcategory $\category{S}$ is \emph{full} if given two $\category{S}$-objects $X$ and $Y$, any short map from $X$ to $Y$ is a $\category{S}$-morphism.
A \emph{$G$-action on a $\category S$-space} $X$ is simply a group homomorphism $\alpha\colon G\to\Aut_{\category{S}}(X)$.
%It has \emph{bounded orbits} if $F_{\category S}\circ \alpha\colon G\to \Aut_{\category{QPMet}}(X)$ has bounded orbits.
%\end{defn}
%
%
%In practice, we will often simply write $\category{S}$  for the pair $(\category{S},F_{\category{S}})$.
%Since $\PMet$ itself is concrete, that is we have a faithful functor $F\colon \category{QPMet}\to\category{Set}$, the category $\category{S}$ is also concrete (via $F\circ F_S$) and its objects can be thoughts as sets with ``extra structure''.
...
Affine real Hilbert spaces, real Banach spaces and more generally normed spaces are also full subcategories of $\category{Met}$ if we restrict ourself to morphisms that do not increase the distance (that is such that  $\|f(x)\|\leq\| x\|$). In particular, for us isomorphisms of Hilbert and Banach spaces will always be affine isometries.
...
Nevertheless, for our purpose, the choice of the above two embeddings $\category{Graph}\hookrightarrow\category{Met}$ will make no difference.
...
Another example of property of the form \BS{} can be found in \cite[Definition 6.22]{MR1023471}: a group has property~(FHyp$_{\category C}$) if any action on a complex or real hyperbolic space has a bounded orbit. This property is implied by property~(FH), but does not imply property~(FA) \cite[Corollary 6.23 and Example 6.24]{MR1023471}.
...
Then $\Aut_{\category{Graph}}(Z)=\Z\rtimes (\Z/2\Z)$ is the infinite dihedral group and every of its infinite subgroup projects onto $\Z$. Hence, we obtain that a group $G$ has no quotient isomorphic to $\Z$ if and only if every $G$ action on $Z$ has bounded orbits. Let us denote by (B$\Z$) this property.

It follows from Bass-Serre theory that (FA) implies (B$\Z$). This implication is strict as demonstrated by $\category Q$.
In fact, the counterexample $\category Q$ shows that (B$\Z$) does not imply uncountable cofinality, while $\Z$ demonstrate that uncountable cofinality does not implies (B$\Z$).
\end{exmp}
%
%
An example of an uninteresting property \BS{} is given by taking $\category{S}$ to be the category of metric spaces of bounded diameter (together with short maps), in which case any group has \BS.
On the opposite, if $\category{S}$ is the category of extended pseudo-metric spaces ($d$ takes values in $\R_+\cup\{\infty\})$, only the trivial group has \BS.
...
A subcategory $\category{S}$ of $\PMet$ has \emph{unbounded Cartesian powers} if for any $\category S$-space $X$ and any integer $n$, there exists a $\category S$-object, called the \emph{$n$\textsuperscript{th} Cartesian power of $X$} and written $X^n$, such that:
\begin{enumerate}
\item
As a set, $X^n$ is the $n$\textsuperscript{th} Cartesian power of $X$, 
\item\label{Item:Product}
$\Aut_{\category{S}}(X)^n\rtimes \Sym(n)$ is a subgroup of $\Aut_{\category{S}}(X^n)$,
...
%\end{rem}
\PHInline{J'ai totalement réécrit cette sous-section. Tu me diras ce que tu en penses.}
This subsection is devoted to variations and generalizations of property \BS. It is intended as a note for the interested reader, and can be skipped without any harm.\PH{Réécrire l'intro}

First of all, observe that a group $G$ has property \BS{} if and only any $G$ action on a $\category S$ space has at least one bounded orbit.
...
Some of the properties that are of interest for us have been historically defined via the existence of a fixed point for some action. More generally, we say that a group $G$ has \emph{property~(F$\category S$)} if any $G$ action on a $\category S$-space has a fixed point.
...
In the above, we defined property \BS{} for $\category{S}$ a subcategory of $\PMet$.
It is possible to generalize this definition to more general categories.
However, we are not aware of any example of the existence of a group property arising in this general context that is not equivalent to a property \BS{} in the sense of Definition~\ref{Def:PropBS}.

On one hand, we can replace $\PMet$ with a more general category.
For example, one can look at the category $\category{M}$ of sets $X$ endowed with a ``distance function'' $d\colon X\times X\to\R_{\geq 0}$ satisfying the triangle inequality. That is, $d$ is a pseudo-distance, except that is is not necessary symmetric and $d(x,x)$ may be greater than $0$.
All the statements and the proofs remain true for $\category{S}$ a subcategory of $\category{M}$.

On the other hand, we can define property \BS{} for any category $\category{S}$ over $\PMet$, that is for any category~$\category{S}$ endowed with a faithful functor $F\colon\category{S}\to \PMet$. Such a couple $(\category{S},F\colon\category{S}\to \PMet)$ is sometimes called a \emph{structure over $\PMet$}, and $F$ is said to be \emph{forgetful}.
...
An example of such an $\category{S}$ that cannot be expressed as a subcategory of $\PMet$ is the category of edge-labeled graphs, where the morphisms are graph morphisms that induce a permutation on the set of labels.
However, in this case, the property \BS{} is equivalent to the Bergman's property.

One can also combine the above two examples and look at couples $(\category{S},F\colon\category{S}\to \category{M})$, with $F$ faithful.
...
The reason behind this is the forthcoming Lemma~\ref{Lemma:InterProd} and its corollaries, which fail for general bornological spaces.
In fact, all the statements and the proofs remain true for a general $(\category{S},F\colon\category{S}\to \category{Born})$ as soon as $\category{S}$ satisfies Lemma~\ref{Lemma:InterProd}.
Here is an example of such an $\category{S}$. Take $\kappa$ to be an infinite cardinal and let $\category{S_\kappa}$ be the subcategory of $\category{Born}$ where a subset $E$ of a $\category{S_\kappa}$-space is bounded if and only if $\lvert E\rvert<\kappa$.
%\end{rem}
%
%
%
...
%
%
Throughout this section, $\category{S}$ will denote subcategory of $\PMet$ and \BS{} the group property \emph{every action by $\category S$-automorphisms on a $\category{S}$-space has bounded orbits}.
%Heuristically, a $\category{S}$-space is a (quasipseudo-) metric space with an additional structure (as for example an Hilbert space). For a precise definition, see Definition~\ref{Def:PropBS}.
...
\end{lem}
\begin{proof}
We have $H\cong G/N$. If $H$ acts on some $\category{S}$-space $X$ with an unbounded orbit, then the homomorphism $G\twoheadrightarrow H$ gives us a $G$ action on $X$, with the same orbits as the $H$ action.
\end{proof}
%
%
...
\end{lem}
\begin{proof}
Let $X$ be a $\category{S}$-space on which $G$ acts and let $x$ be an element of $X$.
Then the action of $G$ on $X$ restricts to actions on $A$ and $B$.
...
Let $N\rtimes H$ be a 
%topological\PH{Ajouté ``topological''. Ça signifie que l'action de $H$ sur $N$ est continue. On a besoin de cela pour que produit semidirect interne et externe coincide et soient des groupes topologiques}
...
%
%
When $\category{S}$ has a suitable notion of quotients (by a group of isometries), it is possible to obtain a strong version of Lemmas~\ref{Lemma:Quotient} and~\ref{Lemma:InterProd}.
...
%continuously\PH{Ajouté «continuous»}
...
%continuous,\PH{Ajouté «continuous»}
...
%
%
While the statement (and the proof) of Lemma~\ref{Lemma:Cof} is expressed in terms of uncountable cofinality, it is also possible to state it and prove it for a subcategory $\category{S}$ of $\PMet$ without a priori knowing if \BS{} is stronger than having uncountable cofinality.
The main idea is to find a ``natural'' $\category S$-space on which $G=\bigoplus_{i\geq 1}G_i$ acts. For example, for Hilbert spaces, one can take $\bigoplus_{i\geq 1}\ell^2(G_i)$. For connected median graphs, one takes the connected component of $\{1_{G_1},1_{G_2},\dots\}$ in $\powerset{\bigsqcup_{i\geq 1} G_i}$.
...
Let $\category 1\coloneqq\bigcup_{x\in X} 1_{G}$ be the subset of $\powerset{\bigsqcup_XG}$ consisting of the identity elements of all the copies of $G$.
Since every element of $\bigoplus_XG$ has only a finite number of non-trivial coordinates, the action of $G\wr_XH$ preserves the connected components of $\category 1$ (and in fact every connected component of $\powerset{\bigsqcup_XG}$).

Let $I=\{i_1,i_2,\dots\}$ be a countable subset of $X$ and for every $i\in I$, choose a non-trivial $g_i\in G_{i}$.
Then the orbit of the vertex $\category 1$ contains the point $\{g_{i_1},\dots, g_{i_n}\}\cup\bigl(\bigcup_{j>n} 1_{G_{i_j}}\bigr)\cup\bigl(\bigcup_{x\notin I} 1_{G_{x}}\bigr)$ which is at distance $2n$ of~$\category 1$.
Therefore, the action of $G\wr_XH$ on the connected component of~$\category 1$ has an unbounded orbit and then $G\wr_XH$ does not have property~(FW).
\end{proof}
%
%
Once again, given a suitable $\category{S}$, it is sometimes possible to give a direct proof of Lemma~\ref{Lemma:XFinite}.
...
\item
If $\category{S}$ has unbounded Cartesian powers and $G$ has property \BS, then $H$ has property \BS.
\end{enumerate}
\end{lem}
\begin{proof}
Suppose that $G$ does not have \BS{} and let $X$ be a $\category S$-space on which $G$ acts with an unbounded orbit $\orbite$.
...
Let $\alpha\colon H\curvearrowright X$ be an action of $H$ on a $\category S$-space $(X,d_X)$ such that there is an unbounded orbit $\orbite$.
Similarly to the classical theory of representations of finite groups, we have the induced  action $\Ind_H^G(\alpha)\colon G \curvearrowright X^{G/H}$ on the set $X^{G/H}$. Since $H$ has finite index, $X^{G/H}$ is a $\category S$-space and the action is by $\category{S}$-automorphisms. On the other hand, the subgroup $H\leq G$ acts diagonally on $X^{G/H}$, which implies that $\diag(\orbite)$ is contained in a $G$-orbit.
...
We then define $g.(x_1,\dots,x_n)\coloneqq(h_{g,g^{-1}.1}.x_{g^{-1}.1},\dots,h_{g,g^{-1}.n}.x_{g^{-1}.n})$. This is indeed an action by $\category{S}$-auto\-mor\-phisms on $X^{G/H}$ by Condition~\ref{Item:Product} of Definition~\ref{Def:Cartesian}.
...
Suppose that $\category{S}$ has unbounded Cartesian powers. If $X$ is finite and $G\wr_XH$ has property \BS, then $G$ has property \BS.
\end{lem}
\begin{proof}
Suppose that $G$ does not have \BS{} and let $(Y,d_Y)$ be a $\category S$-space on which $G$ acts with an unbounded orbit $G.y$.
Then $(Y^X,d)$ is a $\category S$-space and we have the \emph{primitive action} of the wreath product $G\wr_XH$ on $Y^X$:
\[
	\bigl((\varphi,h).\psi\bigr)(x)=\varphi(h^{-1}.x).\psi(h^{-1}.x).
\]
By Condition~\ref{Item:Product} of Definition~\ref{Def:Cartesian}, this action is by $\category{S}$-automorphisms.
...
Suppose that $\category{S}$ has unbounded Cartesian powers and is such that \BS{} implies (FW).
Let $G$ and $H$ be two groups with $G$ non-trivial and let $X$ be a set on which $H$ acts. Then the wreath product $G \wr_X H$ has property \BS{} if and only if $G$ and $H$ have property \BS{} and $X$ is finite.
\end{thm}
%
%
