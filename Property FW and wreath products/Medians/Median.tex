...
%
\begin{lem}
A group $G$ has the Bergman's Property if and only if any $G$ action on a quasipseudo-metric space has bounded orbits.
\end{lem}
\begin{proof}
...
A qp-metric structure $(\mathbf S,F_{\mathbf S})$ has \emph{bounded cartesian powers} if for any $\mathbf S$-space $X$ and any integer $n$, there exists a $\mathbf S$-object, called the \emph{$n$\textsuperscript{th} cartesian power of $X$} and written $X^n$, such that:
\begin{enumerate}
\item
...
On the other hand, trees do not have bounded cartesian powers.
%
%
\begin{rem}
...
Here is the corresponding result for Bergman's property.\PH{Je n'ai pas mis l'énoncé général, car dans les classes d'espaces qui nous intéressent, seul $(\mathbf{QP})\mathbf{Met}$ est stable par quotient.}
%
%
\begin{prop}\label{Prop:Extension}
Let $1\to N\to G\to H\to 1$ be a group extension.
Then $G$ has the Bergman's property if and only if $H$ has the Bergman's property and the pair $(G,N)$ has the relative Bergman's property.
\end{prop}
\begin{proof}
One direction is simply Lemma~\ref{Lemma:Quotient} and the definition of relative Bergman's property.

On the other hand, let $X$ be a quasipseudo-metric space on which $G$ acts by isometries and let $x$ be an element of $X$.
...
An infinite abelian group either has a quotient which is isomorphic to $\Z$, or a quotient which is an infinite direct sum of non-trivial groups.
