...
All the results cited in this section are standard and can be found in any good book about geometric group theory, see for example~\cite{}.\todo{Mettre ref et peut-être retravailler l'intro.} 
%
%
%
%
...
If $S$ and $T$ are two generating sets of $G$, the graphs $\Sch(G,H;S)$ and $\Sch(G,H;T)$ need not to be isomorphic. However, if both $S$ and $T$ are finite, then $\Sch(G,H;S)$ and $\Sch(G,H;T)$ are quasi-isometric\todo{Put ref ?}, see Figure~\ref{Figure:CayleyOfZ} for some examples. The only fact we will use about quasi-isometries is that these preserve ``large-scale properties'' of the graph, as for examples the number of ends.
...
\end{defn}
There exists other characterization of the number of ends in graphs, notably in terms of equivalence classes of rays see \cite{},\todo{put some reference} but Definition \ref{Def:Ends} is the one that best suits our purpose.
...
While the number of ends of Cayley graphs of finitely generated groups can only be $0$, $1$, $2$ or infinite (in which case it is uncountable) \cite{}\todo{put ref}, Schreier graphs may have any number of ends in $\mathbf N\cup\{\infty\}$, see Figure \ref{Figure:SchreierOfF2}.\todo{Est-ce qu'on dit que tout graphe de degré pair est isomorphique à un Schreier ?}
